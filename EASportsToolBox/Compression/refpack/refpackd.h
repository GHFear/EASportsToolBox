...
namespace refpack
{
    ...
    public:
      inline ByteAccess(bool bigendian = true) {
            this->bigendian = bigendian;
            setEndianFormat();
        }

      inline void setEndianFormat() {
            if (bigendian) {
                efmt = ">";
            }
            else {
      ...
      inline uint32_t getUInt32(const std::string& b, size_t offset = 0) {
            uint32_t value;
            std::memcpy(&value, b.data() + offset, sizeof(uint32_t));
            return value;
        }

      inline float getFloat(const std::string& b, size_t offset = 0) {
            float value;
            std::memcpy(&value, b.data() + offset, sizeof(float));
            return value;
        }

      inline uint16_t getUInt16(const std::string& b, size_t offset = 0) {
            uint16_t value;
            std::memcpy(&value, b.data() + offset, sizeof(uint16_t));
            return value;
        }

      inline uint8_t getUInt8(const std::string& b, size_t offset = 0) {
            uint8_t value;
            std::memcpy(&value, b.data() + offset, sizeof(uint8_t));
            return value;
        }

      inline std::string putUInt32(uint32_t v) {
            std::string buffer(sizeof(uint32_t), '\0');
            std::memcpy(&buffer[0], &v, sizeof(uint32_t));
            return buffer;
        }

      inline std::string putUInt16(uint16_t v) {
    ...
    inline std::unordered_map<int, std::pair<int, int>> QMapTable{
        {0, {0, 0}},
        {1, {3, 1 * 1024 - 1}},
        {2, {3, 16 * 1024 - 1}},
        {3, {1, 1 * 1024 - 1}},
    ...
    // ------------------------------------------------------------------------
    inline int32_t _match_length(const uint8_t* src, int32_t sindex, int32_t dindex, int32_t maxmatch) {
        int32_t current = 0;
    ...
    // ------------------------------------------------------------------------
    inline int32_t _get_hash(const uint8_t* src, int32_t offset) {
    ...
    inline std::string compress(const std::vector<uint8_t>& data, int level, bool littleendian = false) {
    ...
    inline size_t _copyn(const std::vector<uint8_t>& input, size_t sindex, std::vector<uint8_t>& output, size_t dindex, size_t run) {
        while (run > 0) {
    ...
    inline std::tuple<size_t, size_t> get_decompressed_size(const std::vector<uint8_t>& src) {
        size_t size = 0;
        size_t dataoffset = 0;
