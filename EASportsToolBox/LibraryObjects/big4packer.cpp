...
namespace EASportsToolBox
{
	namespace Big4
	{
		...
		bool BuildBHVIV4(const std::vector<Big4Toc>& Big4Toc_vector, std::string BigHeaderPath, uint32_t archive_end_position, bool IsHash64)
		{
			...
			BHHeader.file_count = EABig::Tools::BigToLittleUINT(file_count); // Set file count (Big endian)
			BHHeader.header_size = EABig::Tools::BigToLittleUINT(BHHeader.size); // Set header size. (Big endian)
			...
			if (IsHash64 == true) {
				...
				for (size_t i = 0; i < Big4Toc_vector.size(); i++)
				{
					...
					uint64_t namehash64 = EABig::Hashes::djb2_hash_64(Big4Toc_vector[i].filename.c_str());
					BH_TOC.offset = Big4Toc_vector[i].offset;
					BH_TOC.size = Big4Toc_vector[i].size;
					BH_TOC.decompressed_size = 0;
					BH_TOC.hash = EABig::Tools::BigToLittleUINT(namehash64);
				...
				for (size_t i = 0; i < Big4Toc_vector.size(); i++)
				{
					uint32_t namehash32 = EABig::Hashes::djb2_hash_32(Big4Toc_vector[i].filename.c_str());
					BH_TOC.offset = Big4Toc_vector[i].offset;
					BH_TOC.size = Big4Toc_vector[i].size;
					BH_TOC.decompressed_size = 0;
					BH_TOC.hash = EABig::Tools::BigToLittleUINT(namehash32);
			...
			// BH Footer
			BHFooter footer = {};
			footer.archive_size = EABig::Tools::BigToLittleUINT(archive_end_position);
			footer.unk1 = 0;
			footer.unk2 = 0;
			footer.unk3 = 0;
		...
		bool write_init_header_section(std::vector<std::string>& toc_compatible_filepaths, bool Big4IsBigFCheckState, std::ofstream& outfile)
		{
			...
			header.number_files = EABig::Tools::BigToLittleUINT((uint32_t)toc_compatible_filepaths.size());
		...
		bool update_header_section(const uint32_t& header_end_position, const uint32_t& archive_end_position, std::ofstream& outfile)
		{
			...
			uint32_t header_length = EABig::Tools::BigToLittleUINT(header_end_position);
		...
		auto write_toc_section(std::vector<std::string>& toc_compatible_filepaths, std::ofstream& outfile, const std::vector<std::string>& filepaths)
		{
			...
			for (size_t i = 0; i < filepaths.size(); i++) {
				...
				Big4Toc toc;
				toc.offset = 0;
				toc.size = EABig::Tools::BigToLittleUINT((uint32_t)fs::file_size(filepaths[i]));
		...
		bool update_toc_section(std::vector<Big4Toc>& Big4Toc_vector, const std::vector<uint32_t>& toc_offset_offset_vector, const std::vector<uint32_t>& offset_vector, std::ofstream& outfile)
		{
			...
			for (size_t i = 0; i < toc_offset_offset_vector.size(); i++)
			{
				...
				uint32_t offset = EABig::Tools::BigToLittleUINT(offset_vector[i]);
		...
		bool write_settings_section(std::ofstream& outfile, bool Big4CompressionCheckState)
		{
			...
			settings.DJBHashKey = EABig::Tools::BigToLittleUINT((uint16_t)5381);  // Replace with your desired value (hash key) (Skate and most EA Games uses 5381)
			settings.bIsCompressed = EABig::Tools::BigToLittleUINT(bIsCompressed);
		...
		bool write_filecontent_section(std::ofstream& outfile,
			const std::vector<std::string>& filepaths, std::vector<uint32_t>& offset_vector)
		{
			for (size_t i = 0; i < filepaths.size(); i++) {
				...
				if (EABig::Tools::isFileEmpty(filepaths[i])) {
					continue;
				}
		...
		bool PackBig4(std::vector<std::wstring> InFilePaths, std::wstring InDirectory,
			std::wstring OutFilePath, Big4PackerSettings Settings)
		{
			std::vector<Big4Toc> Big4Toc_vector = {};
			std::wstring top_level_path = EABig::Tools::Strings::removeLastFolder(InDirectory);
			...
			std::string OutArchivePath = EABig::Tools::Strings::WideStringToString(OutFilePath);
			std::string BigHeaderPath = EABig::Tools::Strings::replaceFileExtension(
				EABig::Tools::Strings::WideStringToString(OutFilePath), ".bh");

			for (size_t i = 0; i < InFilePaths.size(); i++)
			{
				std::wstring TOC_Compatible_Path = InFilePaths[i];
				size_t pos = TOC_Compatible_Path.find(top_level_path);

				if (pos != std::wstring::npos) {
					TOC_Compatible_Path = EABig::Tools::Strings::make_big_compatible_paths(
						pos, top_level_path, TOC_Compatible_Path);
				}
				else {
					std::wcout << L"Substring not found." << std::endl;
				}
				toc_compatible_filepaths.push_back(EABig::Tools::Strings::WideStringToString(TOC_Compatible_Path));
				filepaths.push_back(EABig::Tools::Strings::WideStringToString(InFilePaths[i]));
			}
			...
			if (!IsWithinBig4SizeLimit(filepaths))
			{
				MessageBox(0, L"ERROR: Archive would exceed uint32 size limit of 4294967295 bytes!\nBuild process stopped.",
					L"Packer Prompt", MB_OK | MB_ICONERROR);
				return false;
			}
			...
			if (!write_settings_section(outfile,
				Settings.ContainsCompressedFiles)) {
				outfile.close();
				return false;
			}
			...
			if (!BuildBHVIV4(Big4Toc_vector, BigHeaderPath,
				archive_end_position, Settings.IsHash64)) {
				return false;
			}
